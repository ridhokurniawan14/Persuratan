              ...
              <form method="POST" action="{{ route('surat-keluar.store') }}" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="card-body">
                  <div class="form-group row">
                    ...
                    <div class="col-sm-2">
                      <select name="kode_surat_keluar" class="custom-select">
                        ...
                        @foreach ($categories as $category)
                          <option required value="{{ $category->id }}" {{ old('kode_surat_keluar') == $category->id ? 'selected' : '' }}>
                    ...
                    <div class="col-sm-4">
                      <input value="{{ old('tujuan') }}" autocomplete="off" required type="text" name="tujuan" class="form-control @error('tujuan') is-invalid @enderror" id="tujuan" placeholder="Masukkan Tujuan">
                      @error('tujuan')
                        <div class="invalid-feedback">
                          {{ $message }}
                        </div>
